...
# Optional: reset DB to ensure identical starting state across runs.#!/usr/bin/env bash
set -euo pipefail
...
"${ROOT_DIR}/sandbox/up.sh" "${SCENARIO}" | tee "${OUT_DIR}/run.log"
...
# Grab latest fingerprint produced in /tmp (DB-proof export)
...
# Grab latest fingerprint produced in /tmp
# Also persist the source path for traceability.
FP_SRC="$(ls -1t /tmp/e2e_payload_fingerprint_*.csv 2>/dev/null | head -n1 || true)"
if [ -z "${FP_SRC}" ]; then
  log "ERROR: fingerprint CSV not found in /tmp"
  exit 1
...
# Normalize fingerprint to be stable across runs:
# - replace UUIDs with stable tokens UUID1, UUID2, ... based on first-seen order
# - replace time-based idempotency tokens (idem-*-<digits>) with stable tokens IDEM1, IDEM2, ...
python3 - <<'PY' "${OUT_DIR}/payload_fingerprint.csv" "${OUT_DIR}/payload_fingerprint_normalized.csv"
import csv, re, sys
from pathlib import Path
...
idem_any_re = re.compile(
    r"\b[a-z0-9_-]*idem[a-z0-9_-]*-\d+\b",
    re.IGNORECASE,
)
...
with in_path.open("r", newline="") as f_in, out_path.open("w", newline="") as f_out:
    ...
    w = csv.writer(f_out, lineterminator="\n")
    for row in r:
        w.writerow([normalize_cell(cell) for cell in row])
PY



# sha256 helpers (raw + normalized)
sha256_file() {
  local f="$1" out="$2"
  if command -v sha256sum >/dev/null 2>&1; then
    sha256sum "$f" | awk '{print $1}' > "$out"
